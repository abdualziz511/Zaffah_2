...
export default function AdminPage() {
  ...
  const upsert = async (endpoint, stateList, setStateList, item, isTrack = false) => {
    try {
      ...
      console.error(err);
      showToast('خطأ: ' + (err.message || 'حدث خطأ أثناء الحفظ'));
    }
  };
...
// يرفع الملف عبر /api/upload (Filebase S3) ويُرجع الـ Public URL
// يدعم الصور (image/*) وملفات الصوت (audio/*) مع MIME Type صحيح
function FileUpload({ label, currentUrl, onUploaded, bucket = 'cover-images', accept = 'image/*', folder = '' }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      // ── إرسال الملف إلى /api/upload (Server Route → Filebase S3) ──
      // السيرفر يتولى: MIME Type، اسم الملف، المجلد، والرفع إلى Filebase
      ...
      if (!res.ok) {
        throw new Error(result.error || `فشل الرفع (${res.status})`);
      }

      // result.url = رابط Filebase العام → يُحفظ في Supabase PostgreSQL
  ...
  const isImage = bucket.includes('cover-images');
